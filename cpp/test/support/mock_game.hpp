...
static constexpr char game_data[]
  = "\x00\x00\x00\x00"  // 0:  unused
    "\x00\x11"          // 4:  skip(1), level = 17
    "\x00\x00\x00\x00"  // 6:  skip(4)
    "\x7b\x00"          // 10: health = 123, skip(1)
    "\x0b\x16\x00\x00"  // 12: pos = (11, 22, skip(2))
    "\x00"              // 16: target_ptr = 0
    "\x02"              // 17: shop_ptr = 2
    "\x06"              // 18: weapon_ptr = 6
    ...
    "\x2f"              // 22: mana = 47
    "\x00"              // 23: skip(1)
    "\x01\x02\x00\x00"  // 24: hands[0] = (1, 2, skip(2))
    "\x03\x04\x00\x00"  // 28: hands[1] = (1, 2, skip(2))
    "\x2c\x38"          // 32: pets vec (44, 48)
    "\x3c\x00"          // 34: caves circular list = 60, unused(1)
    "\x58\x63\x00\x00"  // 36: prev_pos = (88, 99, skip(2))
    "\x37\x42\x00\x00"  // 40: tagged_pos = (55, 66, skip(2))
    "\x05\x06\x00\x00"  // 44: pets[0] = (5, 6, skip(2))
    "\x07\x08\x00\x00"  // 48: pets[1] = (7, 8, skip(2))
    "\x09\x0a\x00\x00"  // 52: pets[2] = (9, 10, skip(2))
